...
//go:generate minimock -i github.com/kms-qwe/chat-server/internal/service.ChatService -o chat_service_minimock.go -n ChatServiceMock -p mocks
...
// ExpectUsernamesParam2 sets up expected param usernames for ChatService.CreateChat
func (mmCreateChat *mChatServiceMockCreateChat) ExpectUsernamesParam2(usernames []string) *mChatServiceMockCreateChat {
	if mmCreateChat.mock.funcCreateChat != nil {
		mmCreateChat.mock.t.Fatalf("ChatServiceMock.CreateChat mock is already set by Set")
	}
	...
	if mmCreateChat.CreateChatMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.usernames != nil && !minimock.Equal(*mm_want_ptrs.usernames, mm_got.usernames) {
				mmCreateChat.t.Errorf("ChatServiceMock.CreateChat got unexpected parameter usernames, want: %#v, got: %#v%s\n", *mm_want_ptrs.usernames, mm_got.usernames, minimock.Diff(*mm_want_ptrs.usernames, mm_got.usernames))
			}
