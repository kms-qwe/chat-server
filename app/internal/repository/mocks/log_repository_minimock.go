...
//go:generate minimock -i github.com/kms-qwe/chat-server/internal/repository.LogRepository -o log_repository_minimock.go -n LogRepositoryMock -p mocks
...
type LogRepositoryMock struct {
	...
	funcLog          func(ctx context.Context, operation string) (err error)
	inspectFuncLog   func(ctx context.Context, operation string)
...
func (mmLog *mLogRepositoryMockLog) Expect(ctx context.Context, operation string) *mLogRepositoryMockLog {
	...
	mmLog.defaultExpectation.params = &LogRepositoryMockLogParams{ctx, operation}
...
// ExpectOperationParam2 sets up expected param operation for LogRepository.Log
func (mmLog *mLogRepositoryMockLog) ExpectOperationParam2(operation string) *mLogRepositoryMockLog {
	if mmLog.mock.funcLog != nil {
		mmLog.mock.t.Fatalf("LogRepositoryMock.Log mock is already set by Set")
	}
	...
	mmLog.defaultExpectation.paramPtrs.operation = &operation

	return mmLog
}

// Inspect accepts an inspector function that has same arguments as the LogRepository.Log
func (mmLog *mLogRepositoryMockLog) Inspect(f func(ctx context.Context, operation string)) *mLogRepositoryMockLog {
...
func (mmLog *mLogRepositoryMockLog) Set(f func(ctx context.Context, operation string) (err error)) *LogRepositoryMock {
	if mmLog.defaultExpectation != nil {
...
// Then helper
func (mmLog *mLogRepositoryMockLog) When(ctx context.Context, operation string) *LogRepositoryMockLogExpectation {
	if mmLog.mock.funcLog != nil {
		mmLog.mock.t.Fatalf("LogRepositoryMock.Log mock is already set by Set")
	}
	...
	expectation := &LogRepositoryMockLogExpectation{
		mock:   mmLog.mock,
		params: &LogRepositoryMockLogParams{ctx, operation},
	}
...
func (mmLog *LogRepositoryMock) Log(ctx context.Context, operation string) (err error) {
	mm_atomic.AddUint64(&mmLog.beforeLogCounter, 1)
	defer mm_atomic.AddUint64(&mmLog.afterLogCounter, 1)

	if mmLog.inspectFuncLog != nil {
		mmLog.inspectFuncLog(ctx, operation)
	}

	mm_params := LogRepositoryMockLogParams{ctx, operation}
	...
	if mmLog.LogMock.defaultExpectation != nil {
		...
		mm_got := LogRepositoryMockLogParams{ctx, operation}
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.operation != nil && !minimock.Equal(*mm_want_ptrs.operation, mm_got.operation) {
				mmLog.t.Errorf("LogRepositoryMock.Log got unexpected parameter operation, want: %#v, got: %#v%s\n", *mm_want_ptrs.operation, mm_got.operation, minimock.Diff(*mm_want_ptrs.operation, mm_got.operation))
			}
	...
	if mmLog.funcLog != nil {
		return mmLog.funcLog(ctx, operation)
	}
	mmLog.t.Fatalf("Unexpected call to LogRepositoryMock.Log. %v %v", ctx, operation)
	return
}
